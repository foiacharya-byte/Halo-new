...
const REASONS: { value: EarlyAccessReason; label: string; hint: string }[] = [
  { value: "need_help", label: "I want to discover Vadodara", hint: "Find what Baroda actually trusts" },
...
export function WaitlistModal({ open, onClose, initialRole }: WaitlistModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-3 sm:p-8" role="dialog" aria-modal="true" aria-label="Join the Halo waitlist">
      <div className="absolute inset-0 bg-[#08040280] backdrop-blur-md" onClick={handleClose} />

      <motion.div
        initial={{ opacity: 0, y: 22, scale: 0.98 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ duration: 0.4, ease: [0.22, 0.61, 0.36, 1] }}
        className="relative grid w-full max-w-3xl overflow-hidden rounded-[22px] border border-gold/15 bg-paper-deep shadow-2xl sm:grid-cols-[0.78fr_1fr] max-h-[94vh] max-sm:max-h-[100dvh] max-sm:rounded-none"
      >
        {/* Cinematic rail — desktop only */}
        <div
          className="relative hidden min-h-[520px] flex-col justify-between overflow-hidden p-8 sm:flex"
          style={{
            background:
              "radial-gradient(130% 80% at 25% 15%, rgba(194,83,31,.4), transparent 52%), linear-gradient(158deg,#1d1107 0%,#0d0804 56%,#170e05 100%)",
          }}
        >
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-br from-[#120b05]/50 via-[#120b05]/32 to-[#0d0804]/90" />
          <div className="relative z-10 flex h-9 items-center">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/assets/halo/vadodara/supplied/logo-ae-halo.png" alt="ae halo" className="h-9 w-auto drop-shadow-lg" />
          </div>
          <div className="relative z-10">
            <p className="font-display text-[28px] font-medium leading-tight tracking-tight text-[#F4ECDC]">
              Baroda is building<br />
              <em className="italic text-[#E0A867]">its own place.</em>
            </p>
            <p className="mt-3.5 max-w-[30ch] text-xs leading-relaxed text-ink-soft">
              ...
              {STEPS.map((s, i) => (
                <div
                  ...
                  className={
                    ...
                    (i === step ? "text-[#F4ECDC]" : i < step ? "text-gold" : "text-ink-faint")
                  }
                >
                  <span
                    className={
                      "h-1.5 w-1.5 shrink-0 rounded-full transition-colors " +
                      (i === step ? "bg-accent" : i < step ? "bg-gold" : "bg-ink-faint/40")
                    }
                  />
                  {s}
                </div>
        ...
        <div className="relative flex min-h-0 flex-col">
          <button
            ...
            onClick={handleClose}
            aria-label="Close"
            className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full border border-ink/15 bg-paper-deep/70 text-ink-soft backdrop-blur transition-colors hover:text-ink"
          >
          ...
          {!done && (
            <>
              ...
              <div className="mx-8 mt-4 h-0.5 overflow-hidden rounded-full bg-ink/10">
                <div
              ...
              {done ? (
                <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-4 text-center">
                  <div
                    ...
                    style={{ background: "radial-gradient(circle, rgba(212,164,67,.22), rgba(194,83,31,.07) 62%, transparent 76%)" }}
                  >
                    <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#D4A443" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M20 6 9 17l-5-5" />
                    </svg>
                  </div>
                  <h3 className="font-display text-2xl font-medium tracking-tight text-ink">You are part of the first circle.</h3>
                ...
                <motion.div key="s0" initial={{ opacity: 0, x: 26 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -26 }} transition={{ duration: 0.32 }}>
                  <h3 className="font-display text-[26px] font-medium leading-tight tracking-tight text-ink">Why are you here?</h3>
                    ...
                    {REASONS.map((r) => (
                      <button
                        ...
                        className={
                          ...
                          (reason === r.value ? "border-accent bg-accent-soft" : "border-ink/10 bg-ink/[0.025] hover:border-accent/50")
                        }
                      >
                        <span className="min-w-0 flex-1">
                          <span className="block font-display text-[17px] font-medium text-ink">{r.label}</span>
                        ...
                        <span
                          className={
                            ...
                            (reason === r.value ? "border-accent bg-accent text-white" : "border-ink/15 text-ink-faint")
                          }
                        >
                          →
                        </span>
                ...
                <motion.div key="s1" initial={{ opacity: 0, x: 26 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -26 }} transition={{ duration: 0.32 }}>
                  <h3 className="font-display text-[26px] font-medium leading-tight tracking-tight text-ink">A little about you.</h3>
                  ...
                  <div className="mt-5 flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      className="w-full rounded-[10px] border border-ink/10 bg-ink/[0.04] px-3.5 py-3 text-[14.5px] text-ink placeholder:text-ink-faint focus:border-accent focus:outline-none"
                    />
                  </div>
                  ...
                  <div className="mt-4 flex flex-col gap-1.5">
                    ...
                    <select
                      ...
                      className="w-full rounded-[10px] border border-ink/10 bg-ink/[0.04] px-3.5 py-3 text-[14.5px] text-ink focus:border-accent focus:outline-none"
                    >
                    ...
                    {INTERESTS.map((i) => (
                      <button
                        ...
                        className={
                          ...
                          (interests.includes(i) ? "border-accent bg-accent text-white" : "border-ink/10 text-ink-soft hover:border-accent/50")
                        }
                      >
                        {i}
                      </button>
                ...
                <motion.div key="s2" initial={{ opacity: 0, x: 26 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -26 }} transition={{ duration: 0.32 }}>
                  <h3 className="font-display text-[26px] font-medium leading-tight tracking-tight text-ink">Stay in touch.</h3>
